...
DECLARE_UNITTEST_BEGIN(FGMRESConvergencePoisson);

void check_fgmres_convergence_poisson(int points, int nx, int ny, int nz, ValueTypeB final_residual_tolerance)
{
    Resources res;
    {
        Matrix_h A;
        ...
        error_string = "nrm >= final_residual_tolerance";
        UNITTEST_ASSERT_TRUE_DESC(error_string.c_str(), nrm[0] / nrm_ini[0] < final_residual_tolerance);
    }
}
...
void run()
{
    // This test checks that fgmres converges in at most N iterations (N is number of rows) for any scalar matrices.
    ValueTypeB final_residual_tol = 1.e-5;
    int min_size = 5;
    int max_size = 10;
    ...
    for (int i = min_size; i <= max_size; i++)
    {
        // Works
        // 2D poisson
        check_fgmres_convergence_poisson(5, i, i, i, final_residual_tol);
        check_fgmres_convergence_poisson(9, i, i, i, final_residual_tol);
    }
}

DECLARE_UNITTEST_END(FGMRESConvergencePoisson);
...
FGMRESConvergencePoisson <TemplateMode<AMGX_mode_dDDI>::Type>  FGMRESConvergencePoisson_instance_mode_dDDI;
FGMRESConvergencePoisson <TemplateMode<AMGX_mode_dFFI>::Type>  FGMRESConvergencePoisson_instance_mode_dFFI;
